...
function semanticScore(
  ...
  for (const word of narrativeWords) {
    ...
    for (const [_, synonyms] of Object.entries(CONCEPT_THESAURUS)) {
    ...
    for (const s of result.suggestions) {
      ...
      if (matchedSection) {
        suggestions.push({
          ...
          sectionNumber: s.sectionNumber || matchedSection.sectionNumber,
          act: s.act || matchedSection.act,
          title: s.title || matchedSection.title,
          confidence: Math.min(0.98, Math.max(0.5, Number(s.confidence) || 0.7)),
          reasoning: s.reasoning || 'Matched by AI analysis of complaint narrative.',
...
export async function findJudgments(crimeType: string, sectionIds: string[]): Promise<AIJudgmentResult[]> {
  ...
  try {
    ...
    if (Array.isArray(result.judgments)) {
      return result.judgments.map((j: any, i: number) => ({
        id: `ai-judgment-${i}`,
        title: j.title || 'Unknown Case',
